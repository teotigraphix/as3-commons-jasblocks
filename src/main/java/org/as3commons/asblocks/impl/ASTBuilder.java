...
public class ASTBuilder
{
	...
	public static void assertNoParent(String astDescription, LinkedListTree ast)
	{
		...
		{
			throw new ASBlocksSyntaxError(astDescription
					+ " cannot be added to a second parent node");
		}
	}

	public static LinkedListTree newMetadataTag(String name)
	{
		LinkedListTree ast = ASTUtils.newParentheticAST(AS3Parser.METATAG, AS3Parser.LBRACK, "[", AS3Parser.RBRACK, "]");
	...
	public static LinkedListTree newBlock(int type)
	{
	...
	static LinkedListTree parenthise(LinkedListTree expr)
	{
	...
	static int precidence(LinkedListTree ast)
	{
		switch (ast.getType())
		{
	...
	static void spaceEitherSide(LinkedListToken token)
	{
